...
float getShapeArea(const Shape* myShape) // myShape = Circle or Rectangle or Triangle
{
...
int main()
{
	...
	for (int shapeArrayIndex = 0; shapeArrayIndex < numberOfShape; shapeArrayIndex++) {
		if (shapeTypeFlag == 0) {
			...
			cin >> radius;
			shapes[shapeArrayIndex] = new Circle(radius, 1, 7);
			shapeTypeFlag++;
		}
			...
			cin >> length >> width;
			shapes[shapeArrayIndex] = new Rectangle(length, width, 1, 7);
			shapeTypeFlag++;
		}
		else {
			...
			cin >> length >> height;
			shapes[shapeArrayIndex] = new Triangle(length, height, 1, 7);
			shapeTypeFlag = 0;
		}
	}
	char choice;
	do {
		...
		case 'b':
			for (int i = 0; i < numberOfShape; i++) {
				cout << i + 1 << ") " << shapes[i]->getArea() << endl;
			}
			break;
		case 'q':
			break;
	...
	cout << "Part 2" << endl;
	cout << "*************************************************" << endl;
	Circle c1(4, 1, 8), c2(5, 1, 6);
	Rectangle r1(3, 3, 3, 3), r2(4, 4, 4, 4);
	Triangle t1(5, 5, 7, 8), t2(6, 6, 8, 9);
